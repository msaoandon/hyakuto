...
export function AudioProvider() {
  ...
  // usePathname() always gives the full URL. Playable routes:
  //   chat /story/chat/<day>/<thread> · VN /story/chat/<day>/vn/<id> ·
  //   DM /story/dms/<thread>. All resolve music the same way (cue ▸ OST ▸
  //   default), so capture the trailing thread id across all three shapes.
  const pathname = usePathname();
  const thread =
    pathname.match(/^\/story\/(?:chat\/[^/]+\/(?:vn\/)?|dms\/)([^/]+)\/?$/)?.[1] ?? null;
