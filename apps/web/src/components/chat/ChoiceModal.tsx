        ...
        <div className="flex flex-col gap-2">
          {options.map((opt, i) => (
            <button
              key={i}
              onClick={() => onChoose(i)}
              className="text-left text-base px-4 py-3 bg-lavelnder-veil text-ink-black rounded-xl border border-beige/20 text-beige hover:border-gold/50 hover:bg-gold/5 transition-colors"
            >
              {opt.text}
            </button>
          ))}
